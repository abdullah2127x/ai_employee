...
"""
task_template.py — updated build_output_file

Changes in v3.0:
- Added `draft_reply` section rendered only for message-type tasks
  (email, whatsapp, linkedin, sms) when Claude returns a non-null draft_reply
- Added `filtered` status mapping for filtered/ignored emails
- Cleaner section layout — approval block, draft block, and footer are
  all conditional and never leave blank sections
- `category: filtered` gets a compact layout (no draft, no approval block)
"""
...
# Task types that can have an outbound reply drafted
REPLY_TASK_TYPES = {"email", "whatsapp", "linkedin", "sms"}
# ============================================================================
# INTERNAL UTILITIES
# ============================================================================
...
def build_output_file(
    ...
    """
    Build the universal output markdown file from Claude's JSON decision.

    Works for all task types: email, file_drop, whatsapp, and future watchers.
    The draft_reply section only renders for message-type tasks when
    Claude returned a non-null draft_reply.

    PATH FIX: original_path_obsidian already contains [[Drop_History/file.ext]]
    as stored in the task file frontmatter. Embed directly — no extra [[ ]].

    Args:
        task_id:                 Unique task identifier
        task_type:               "email" | "file_drop" | "whatsapp" | etc.
        original_name:           Original file/task name or email subject
        original_path_obsidian:  Already-formatted Obsidian link from frontmatter
        decision:                Claude's validated JSON dict (7 fields including draft_reply)
        processed_at:            Processing timestamp
    ...
    ai_decision = decision.get("decision", "unknown")
    ai_category = decision.get("category", "unknown")
    status = status_map.get(ai_decision, "unknown")

    # Filtered emails get a compact status
    if ai_category == "filtered":
        status = "filtered"

    processed_str = processed_at.strftime("%Y-%m-%d %H:%M:%S")
    processed_iso = processed_at.isoformat()

    # ── Draft reply section (message tasks only) ─────────────────────────────
    draft_reply = decision.get("draft_reply")
    draft_section = ""

    if task_type in REPLY_TASK_TYPES and draft_reply:
        draft_section = f"""
---

## Draft Reply

> This reply is ready to send. {'**Awaiting approval before sending.**' if ai_decision == 'create_approval_request' else '**Auto-sent.**'}
...
"""
    ...
    # ── Approval section ─────────────────────────────────────────────────────
    approval_section = ""
    if ai_decision == "create_approval_request":
        approval_section = f"""
---
...
**To action this:**
- Move to `Approved/` → Abdullah reviewed and approved
- Move to `Rejected/` → Do not proceed
- Move to `Needs_Revision/` with a comment → Request changes
"""

    # ── Filtered layout (compact, no noise) ──────────────────────────────────
    if ai_category == "filtered":
        output_md = f"""---
type: {task_type}_result
task_id: {task_id}
...
ai_category: filtered
---

# [Filtered] {original_name}

**Processed:** {processed_str}
**Reason:** {decision.get("action_taken", "Filtered by AI Employee")}

---

## Notes

{decision.get("response", "No additional notes.")}
...
"""
    ...
    # ── Standard layout (all other tasks) ────────────────────────────────────
    output_md = f"""---
type: {task_type}_result
task_id: {task_id}
original_name: {original_name}
...
**Processed:** {processed_str}
**Decision:** {ai_decision.replace("_", " ").title()}
**Category:** {ai_category.title()}
...
{decision.get("response", "No response provided")}
{draft_section}{approval_section}
---
