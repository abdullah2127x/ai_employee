...
VALID_CATEGORIES = {
    ...
    "document",
    "urgent",
    "filtered",  # NEW: for ignored/promotional emails
}

# Task types that support draft_reply (message-based watchers)
...
SKILL_MAP = {
    ...
    "email": ".claude/skills/process-email/SKILL.md",
    "whatsapp": ".claude/skills/process-whatsapp/SKILL.md",
    "linkedin": ".claude/skills/process-linkedin/SKILL.md",
}


def build_prompt(task_file, task_content: str, task_type: str) -> str:
    """
    Build the prompt sent to Claude Code.

    Structure:
    1. Task type and skill pointer — tells Claude exactly what it's processing
    2. Task content — the full filled template
    3. Output requirements — reminds Claude of the exact schema expected
...
Instructions:
1. Read Business_Goals.md and Company_Handbook.md
2. Read the skill file at: {skill_path}
3. Process the task above following the skill rules and CLAUDE.md decision rules
4. Return ONLY the JSON object — 7 fields, no markdown, no explanation:
   decision, category, summary, action_taken, response, draft_reply, approval_reason"""
...
def parse_and_validate(stdout: str, task_type: str = "") -> dict:
    """
    Extract and validate Claude's JSON from stdout.

    v3.0 changes:
    - Validates 7 fields (added draft_reply)
    - draft_reply must be null for filtered category
    - draft_reply must be null for non-message task types
    - "filtered" is a valid category
    - approval_reason logic unchanged

    Args:
        stdout:    Claude's raw stdout
        task_type: The task type string (e.g. "email") for draft_reply validation
    ...
    # Must be null for: filtered emails, non-message task types, phishing flags
    if decision["category"] == "filtered":
        decision["draft_reply"] = None  # Force null for filtered — no reply ever

    if task_type and task_type not in REPLY_TASK_TYPES:
        decision["draft_reply"] = None  # Force null for non-message task types

    # Normalize null-like draft_reply values
    if decision.get("draft_reply") in ("null", ""):
